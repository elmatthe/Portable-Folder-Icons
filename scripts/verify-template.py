...
NOTE ON A FRESH SCAFFOLD: this gate is DESIGNED to FAIL right after scaffolding
and keep failing until real content exists. Specifically the docs check (below)
requires Briefing.md to be de-templated with real content and Changelog.md to
carry a real version entry — a brand-new repo still has template placeholders,
so `verify` will report FAIL on "docs" (and on "pytest" until the first test is
added). That is the intended signal, not a bug: the first real commit is the
one that makes the gate pass. Do not treat the initial red as something to work
around — fill in Briefing.md and add a test, and it goes green.
